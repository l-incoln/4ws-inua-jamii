...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Dynamic event pages
  const eventPages: MetadataRoute.Sitemap = (eventsResult.data ?? []).map((e) => ({
  ...
  const postPages: MetadataRoute.Sitemap = (postsResult.data ?? []).map((p) => ({
  ...
  const programPages: MetadataRoute.Sitemap = (programsResult.data ?? []).map((p) => ({
